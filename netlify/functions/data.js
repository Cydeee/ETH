...
async function buildDashboardData() {
  ...
  try {
    ...
    const oiHist = await safeJson(`https://api.binance.com/futures/data/openInterestHist?symbol=${SYMBOL}&period=1h&limit=500`);
  ...
  try {
    ...
    for (let i = window; i < oneM.length - window; i++) {
      ...
      if (currH === maxH && ((currH - minL)/minL*100) >= 0.15) swingsArr.push({type:'H',idx:i,price:+currH.toFixed(2)});
      if (currL === minL && ((maxH - currL)/maxH*100) >= 0.15) swingsArr.push({type:'L',idx:i,price:+currL.toFixed(2)});
    }
    ...
    if (swingsArr.length>=2) {
      ...
      if (last2[0].type===last2[1].type) {
        if (last2[0].type==='H') { H2=last2[0].price; H1=last2[1].price; }
        else                  { L2=last2[0].price; L1=last2[1].price; }
      }
    }
    if (H1!==null) {
      const [p2,p1]=[swingsArr[swingsArr.length-2].idx, swingsArr[swingsArr.length-1].idx];
