/*  Netlify Node 18 Edge Function – data.js
    ...
    Includes: 4 h ADX-14, abs./percentile OI, sessionRelVol, cycle-anchored VWAP,
...
/* Wilder-ADX (p = 14) – only used on 4 h TF */
...
async function buildDashboardData() {
  const SYMBOL = "ETHUSDT";            // ← changed from BTC
  const LIMIT  = 250;
  ...
  try {
    ...
    out.dataE={stressIndex:+stress.toFixed(2),highRisk:stress>=5,components:{biasScore:b,levScore:l,volScore:v,liqScore:q}};
  } catch(e){ out.dataE=null; out.errors.push(`E: ${e.message}`); }

  /* F – Structure, VPVR, swings, levels */
  try {
    ...
    out.dataF={vpvr:{"4h":vp(h4),"1d":vp(d1),"1w":vp(w1)},price:+price.toFixed(2)};

    /* cycle-anchored VWAP (lowest weekly close last 52 w) */
    ...
    const highs=oneM.map(r=>+r[2]), lows=oneM.map(r=>+r[3]), closes=oneM.map(r=>+r[4]);
    const swings=[]; const W=6;
    for(let i=W;i<oneM.length-W;i++){
      const maxH=Math.max(...highs.slice(i-W,i+W+1));
      const minL=Math.min(...lows.slice(i-W,i+W+1));
      if(highs[i]===maxH && ( (maxH-minL)/minL*100)>=0.15) swings.push({t:"H",i,p:+highs[i].toFixed(2)});
      if(lows[i]===minL && ( (maxH-minL)/maxH*100)>=0.15)  swings.push({t:"L",i,p:+lows[i].toFixed(2)});
    }
    let H1=null,H2=null,L1=null,L2=null,neck=null,neckBreak=false;
    if(swings.length>=2&&swings.at(-1).t===swings.at(-2).t){
      if(swings.at(-1).t==="H"){ H2=swings.at(-1).p; H1=swings.at(-2).p;
        neck=+Math.min(...lows.slice(swings.at(-2).i,swings.at(-1).i+1)).toFixed(2);
        neckBreak=closes.at(-2)>neck && closes.at(-1)<neck;
      }else{ L2=swings.at(-1).p; L1=swings.at(-2).p;
        neck=+Math.max(...highs.slice(swings.at(-2).i,swings.at(-1).i+1)).toFixed(2);
        neckBreak=closes.at(-2)<neck && closes.at(-1)>neck;
      }
    }
    out.dataF.swings={H1,H2,L1,L2}; out.dataF.neckline=neck; out.dataF.neckBreak=neckBreak;

    /* Levels: pivot / VWAP band / HH20-LL20 */
    const yesterday=d1.at(-2);
    if(yesterday){
      const yH=+yesterday[2], yL=+yesterday[3], yC=+yesterday[4];
      const pivot=(yH+yL+yC)/3, R1=2*pivot-yL, S1=2*pivot-yH;
      const h20=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1h&limit=20`);
      const HH20=Math.max(...h20.map(b=>+b[2]));
      const LL20=Math.min(...h20.map(b=>+b[3]));
      ...
      vBars.forEach(b=>{
        ...
        vs+=vol; pv+=px*vol; pv2+=px*px*vol;
      });
